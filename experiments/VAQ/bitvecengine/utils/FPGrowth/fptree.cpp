...
// FPTree::FPTree(CodebookType transactions, uint64_t minimum_support_threshold, const std::vector<int> &centroidsNum) :
//         root( std::make_shared<FPNode>( Item{}, nullptr ) ), header_table(),
//         minimum_support_threshold( minimum_support_threshold )
// {
//     RowVector<uint> offsets(centroidsNum.size());
//     offsets(0) = 0;
//     for (int i=1; i<(int)centroidsNum.size(); i++) {
//         offsets(i) = centroidsNum[i] + offsets(i-1);
//     }

//     // scan the transactions counting the frequency of each item
//     std::map<Item, uint64_t> frequency_by_item;
//     {
//         auto transptr = transactions.data();
//         for ( int i=0; i<(int)transactions.size(); i++) {
//             // preprocess transactions
//             /*
//             [Sub1] = 0..15	[Sub2] = 16..31
...
//             [C1.16]        [C2.16]
//             */
//             ++frequency_by_item[(*transptr) + offsets(i % transactions.cols())];
//             transptr++;
//         }
//     }
...
//     // scan the transactions again
//     for ( int i=0; i<transactions.rows(); i++ ) {
//         auto curr_fpnode = root;
//         RowVector<Item> temp = transactions.row(i).cast<Item>() + offsets;
...
//             // check if item is contained in the current transaction
//             if ( std::find( temp.data(), temp.data() + transactions.cols(), item ) != temp.data() + transactions.cols() ) {
//                 // insert item in the tree
...
// SetPattern fptree_growth(const FPTree& fptree)
// {
...
//         SetPattern single_path_patterns;
...
//         SetPattern multi_path_patterns;
