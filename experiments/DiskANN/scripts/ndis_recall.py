...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Plot DiskANN distance-computations vs recall curves.")
    ...
    parser.add_argument("--output-name", default=None, help="Optional output filename. Defaults to <dataset>_ndis_recall_<target>.pdf")
...
def main() -> None:
    ...
    if args.output_name is None:
        output_name = f"{args.dataset}_ndis_recall_{args.target_recall:.2f}.pdf"
    else:
    ...
    for algo_name, algo_folder in ALGORITHMS.items():
        ...
        entries = [e for e in entries if e.recall >= args.recall_threshold and e.ndis is not None]
        if not entries:
            print(f"[SKIP] {algo_name}: no entries with ndis data above recall threshold")
            continue

        recalls = [entry.recall for entry in entries]
        ndis_vals = [entry.ndis for entry in entries]
