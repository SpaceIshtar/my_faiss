                                                 ...
                                                 QueryStats *stats)
{
    ...
    {
        // Original PQ preprocessing: center the query and rotate if we have a rotation matrix
    ...
    auto compute_dists = [this, pq_coord_scratch, pq_dists, &faiss_dc](const uint32_t *ids, const uint64_t n_ids,
        ...
        if (this->_use_dc_factory || this->_use_faiss_quantization)
        {
            ...
            for (; i + 4 <= n_ids; i += 4)
            {
                faiss_dc->distances_batch_4(ids[i], ids[i + 1], ids[i + 2], ids[i + 3], dists_out[i], dists_out[i + 1],
                                            dists_out[i + 2], dists_out[i + 3]);
            }
            // Process remaining elements
            for (; i < n_ids; i++)
            {
                dists_out[i] = (*faiss_dc)(ids[i]);
            }
        }
        else
        {
            // Original PQ distance computation
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::set_faiss_index(faiss::IndexFlatCodes *faiss_index)
{
...
template <typename T, typename LabelT> bool PQFlashIndex<T, LabelT>::using_faiss_quantization() const
{
    return _use_faiss_quantization;
}
